...
class VerClientes(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {
                    "mensaje_de_error": "No existen clientes o no han podido ser encontrados",
                    "excepcion": str(e),
                },
...
class VerCliente(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            return Response(
                {
                    "mensaje_de_error": "No existe el cliente o no ha podido ser encontrado",
                    "excepcion": str(e),
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                serializer.save()
                return Response(
                    {"nuevo_cliente": serializer.data}, status=status.HTTP_201_CREATED
                )
            else:
                return Response(
    ...
    def put(self, request, id):
        try:
            ...
            if serializer.is_valid():
                serializer.save()
                return Response(
                    {"cliente_editado": serializer.data}, status=status.HTTP_200_OK
                )
            else:
                return Response(
                    {"mensaje_de_error": serializer.errors},
